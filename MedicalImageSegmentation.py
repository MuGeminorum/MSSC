...
def find_new_voxels(sx, sy, sz, iteration):
    ...
    # Add your code below    #
    #########################
    new_voxel_coordinates_yz = offset(
        (
            np.array([sx - iteration, sx + iteration]),
            np.arange(sy - iteration, sy + iteration + 1),
            np.arange(sz - iteration, sz + iteration + 1),
        )
    )
    new_voxel_coordinates_xz = offset(
        (
            np.arange(sx - iteration + 1, sx + iteration),
            np.array([sy - iteration, sy + iteration]),
            np.arange(sz - iteration, sz + iteration + 1),
        )
    )
    new_voxel_coordinates_xy = offset(
        (
            np.arange(sx - iteration + 1, sx + iteration),
    ...
    new_voxel_coordinates = np.concatenate(
        (
            new_voxel_coordinates_yz,
            np.concatenate((new_voxel_coordinates_xz, new_voxel_coordinates_xy)),
        )
    )
    #########################
    # Add your code above    #
    ...
    print(
        "[Iter {}] the shape of new_voxel_coordinates in this iteration: {}".format(
...
def grow_from_seed(
    ...
    while 1 > 0:
        ...
        iteration += 1
        searching_extend = np.array(
            [
                iteration + radius - sx,
                sx + iteration + radius + 1 - dx,
                iteration + radius - sy,
                sy + iteration + radius + 1 - dy,
                iteration + radius - sz,
                sz + iteration + radius + 1 - dz,
            ]
        )
        ...
        new_voxel_values = inputVolumeData[
            new_voxel_coords[:, 0], new_voxel_coords[:, 1], new_voxel_coords[:, 2]
        ]
        glb_voxel_indicators = np.where(
            np.logical_and(new_voxel_values < ROI_max, new_voxel_values > ROI_min)
        )
        ...
        if not glb_voxel_indicators:
            ...
            for i in glb_voxel_indicators[0]:
                ...
                patch_boolen = outputROIData[
                    nx - 1 : nx + 2, ny - 1 : ny + 2, nz - 1 : nz + 2
                ]

                if patch_boolen.sum() > 1:
                    local_value = inputVolumeData[nx, ny, nz]
                    patch_values = inputVolumeData[
                        nx - 1 : nx + 2, ny - 1 : ny + 2, nz - 1 : nz + 2
                    ]
...
class MedicalImageSegmentationWidget:
    ...
    def setup(self):
        ...
        chooseGlobalFrame, chooseGlobalSlider, chooseGlobalSliderSpinBox = (
            numericInputFrame(
                self.parent,
        ...
        chooseLocalFrame, chooseLocalSlider, chooseLocalSliderSpinBox = (
            numericInputFrame(
                self.parent,
                "Maximum Local Intensity Difference:     ",
                "Determine the local range of of intensity values",
                0,
                50,
                1,
                0,
        ...
        connect(
            chooseGlobalSlider, "valueChanged(double)", "state.maxGlobalDiff = args[0]"
        )
        connect(
            chooseGlobalSliderSpinBox,
            "valueChanged(double)",
            "state.maxGlobalDiff = args[0]",
        )
        connect(
            chooseLocalSlider, "valueChanged(double)", "state.maxLocalDiff = args[0]"
        )
        connect(
            chooseLocalSliderSpinBox,
            "valueChanged(double)",
    ...
    def onApply(self):
        ...
        outputROIData = grow_from_seed(
            inputVolumeData,
            [dx, dy, dz],
            [sx, sy, sz],
            [ROI_min, ROI_max],
            self.state.maxLocalDiff,
            outputROIData,
        )
    ...
    def onReload(self, moduleName="MedicalImageSegmentation"):
        ...
        parent = (
            slicer.util.findChildren(name="%s Reload" % moduleName)[0].parent().parent()
        )
